...
class CutsceneCommunication(ScreenSprite):
    ...
    def __init__(self, character_image, position=(0, 0),typing_error_rate=0.0,typing_speed=7,speaker_name=""):
        super().__init__(position=position)
        self.character_image = get_sprite_store().get_sprite(character_image)
        textbox_y_size=self.character_image.get_height() if self.character_image else 100
        ...
        self.elapsed_time=0
        self.typing_error=0
        self.error_probability=typing_error_rate  #10% chance of error per character
        self.typed_text=""
    ...
    def update(self, time_delta):        
        typing_speed=self.typing_speed  #characters per second
    ...
    def draw(self, surface,camera=None):       
        ...
        if self.character_image:
            ...
            total_height=max(self.character_image.get_height()+border_buffer,self.text_box.size[1])
        ...
        if self.speaker_name:
            #draw speaker name above text box
            font = pygame.font.SysFont("arial", 16)
            name_surface = font.render(self.speaker_name, True, (255, 255, 255))
            surface.blit(name_surface, (self.text_box.position[0], self.text_box.position[1]-20))
